...
void app_main(void) {
  /* 0. I2C 总线绝对优先初始化
     因为 wavesahre_rgb_lcd_bl_off 内部也会调用 I2C (ensure_i2c_ready)
     控制背光芯片， 必须赶在它之前让 i2c_bus_create 取得绝对控制权。 */
  // i2c_sensor_aht20_init();
  ...
  /* 6. 临时麦克风测试任务 */
  BaseType_t ret_task = xTaskCreate(mic_test_task, "mic_test", 4096, NULL, 5, NULL);
  if (ret_task != pdPASS) {
