/**
 * @file ui_main.c
 * @brief 提供 LVGL 主界面布局、图像渲染和组件更新逻辑。
 *
 * 涉及多线程环境下的 UI 更新，采用互斥锁保护机制防止并发访问导致的竞争问题。
 */
#include "ui_main.h"
#include "esp_log.h"
...
static lv_obj_t *loading_label;        /* 极简开机黑底等待标签 */
static lv_timer_t *loading_timer;      /* 极简开机等待点动画定时器 */
...
static void _set_photo_locked(uint8_t *data, const char *caption,
  ...
  if (photo_info_label && date) {
    // 1.
    // 日期格式化：统一将任何可能存在的点号、斜杠或单引号规范化为最优雅端正的标准连字符
    // '-'
...
void ui_main_init(void) {
  ...
  /* 创建开机极简黑底等待界面动画，不在此瞬间点亮屏幕背光，而是等首帧渲染完毕 */
  ...
  lv_obj_align(loading_label, LV_ALIGN_CENTER, 0, 0); /* 居于屏幕最中央 */

  loading_timer = lv_timer_create(loading_timer_cb, 500,
                                  NULL); /* 每 500ms 动态呼吸步进一次点数 */
