...
void peripheral_aht20()
{
    ...
    esp_err_t ret = aht20_read_temperature_humidity(aht20, &temperature_raw, &temperature, &humidity_raw, &humidity);

    if (ret == ESP_OK) {
        g_latest_temp = temperature;
        g_latest_hum = humidity;
        g_data_valid = true;
        ESP_LOGI(TAG, "humidity: %2.2f %%  temperature: %2.2f degC", humidity, temperature);
    } else {
